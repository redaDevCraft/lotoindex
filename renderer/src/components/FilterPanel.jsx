...
export default function FilterPanel({ makes, onFilter }) {
  ...
  const activeCount = [makeId, modelId, submodelId, yearFrom, yearTo,
    fuel, gearbox, powerMin, powerMax, mileageMax].filter(Boolean).length

  // Load models when make changes
  useEffect(() => {
    setModelId(''); setSubmodelId(''); setSubmodels([])
    if (makeId) {
      window.db.getModels(makeId).then(setModels)
    } else {
      setModels([])
    }
  }, [makeId])
  ...
  useEffect(() => {
    setSubmodelId([])
    if (modelId) {
      window.db.getSubmodels(modelId).then(setSubmodels)
    } else {
      setSubmodels([])
    }
  }, [modelId])
        ...
        <div className="filter-body">
          ...
          <div className="filter-row">
            ...
            <div className="filter-group">
              ...
              <select
                value={submodelId}
                onChange={e => setSubmodelId(e.target.value)}
                disabled={!modelId}
              >
                <option value="">Tous</option>
                {submodels.map(s => (
